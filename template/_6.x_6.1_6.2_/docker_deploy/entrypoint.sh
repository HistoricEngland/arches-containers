...
setup_postgis_template(){

	echo "Creating PostGIS template"
  	psql --host=${PGHOST} --port=${PGPORT} --user=${PGUSERNAME} --dbname=postgres -c "UPDATE pg_database SET datistemplate='false' WHERE datname='template_postgis';"
   	psql --host=${PGHOST} --port=${PGPORT} --user=${PGUSERNAME} --dbname=postgres -c "DROP DATABASE IF EXISTS template_postgis;"
	psql --host=${PGHOST} --port=${PGPORT} --user=${PGUSERNAME} --dbname=postgres -c "CREATE DATABASE template_postgis"
	psql --host=${PGHOST} --port=${PGPORT} --user=${PGUSERNAME} --dbname=postgres -c "UPDATE pg_database SET datistemplate = TRUE WHERE datname = 'template_postgis'"
	psql --host=${PGHOST} --port=${PGPORT} --user=${PGUSERNAME} --dbname=template_postgis -c "CREATE EXTENSION postgis"
	psql --host=${PGHOST} --port=${PGPORT} --user=${PGUSERNAME} --dbname=template_postgis -c "CREATE EXTENSION postgis_topology"
	psql --host=${PGHOST} --port=${PGPORT} --user=${PGUSERNAME} --dbname=template_postgis -c "GRANT ALL ON geometry_columns TO PUBLIC;"
	psql --host=${PGHOST} --port=${PGPORT} --user=${PGUSERNAME} --dbname=template_postgis -c "GRANT ALL ON spatial_ref_sys TO PUBLIC;"
	psql --host=${PGHOST} --port=${PGPORT} --user=${PGUSERNAME} --dbname=template_postgis -c "GRANT ALL ON geography_columns TO PUBLIC;"
	psql --host=${PGHOST} --port=${PGPORT} --user=${PGUSERNAME} --dbname=template_postgis -c "CREATE EXTENSION IF NOT EXISTS 'uuid-ossp';"
}
...
start_celery_supervisor() {
	...
	supervisord -c /etc/supervisor/arches_supervisord.conf
}
...
run_django_gunicorn(){
	...
	echo ""
	cd ${APP_FOLDER}
	echo "--- compress running...---"
	python3 manage.py compress
	echo "--- collectstatic running...---"
	python3 manage.py collectstatic --noinput &
	echo "--- starting gunicorn ---"
	gunicorn warden.wsgi:application --bind 0.0.0.0:8000 --workers 3
}
